...
# ============================================================================
# Guard Instances (lazy-loaded)
# ============================================================================

_input_guard: Optional[Guard] = None
_data_guard: Optional[Guard] = None
_sql_guard: Optional[Guard] = None


def _get_input_guard() -> Guard:
    """Guard for user message input (prompt injection)."""
    global _input_guard
    if _input_guard is None:
        _input_guard = Guard().use(PromptInjectionValidator())
        logger.info("[OK] Prompt injection guard initialized")
        print("[OK] Prompt injection guard initialized")
    return _input_guard


def _get_data_guard() -> Guard:
    """Guard for data content (indirect prompt injection in CSV data)."""
    global _data_guard
    if _data_guard is None:
        _data_guard = Guard().use(IndirectInjectionValidator())
        logger.info("[OK] Indirect injection guard initialized")
        print("[OK] Indirect injection guard initialized")
    return _data_guard


def _get_sql_guard() -> Guard:
    """Guard for generated SQL output."""
    global _sql_guard
    if _sql_guard is None:
        _sql_guard = Guard().use(SQLSafetyValidator())
...
async def check_input(message: str) -> GuardrailResult:
    ...
    Uses Guardrails AI with a custom LLM-based PromptInjectionValidator.
