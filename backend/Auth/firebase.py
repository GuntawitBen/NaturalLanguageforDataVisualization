...
def init_firebase():
    ...
    try:
        ...
        if firebase_creds_json:
            ...
            logger.info("Firebase initialized from environment variable JSON")
        else:
...
def save_file_to_firebase(user_email: str, file_name: str, table_name: str, row_count: int, column_count: int):
    ...
    try:
        ...
        logger.info(f"File metadata saved to Firebase: {file_name}")
        return True
    except Exception as e:
        logger.error(f"Error saving file to Firebase: {e}")
        return False
...
def get_user_files_from_firebase(email: str):
    ...
    try:
        ...
        logger.error(f"Error getting user files from Firebase: {e}")
        return []