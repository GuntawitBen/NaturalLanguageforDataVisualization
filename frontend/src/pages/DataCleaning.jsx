...
import HeaderPreviewModal from '../components/HeaderPreviewModal';
...
export default function DataCleaning() {
  ...
  const handleUploadSuccess = (tempData) => {
    setPendingUploadData(tempData);
    setShowHeaderPreview(true);
  };

  const handleHeaderConfirm = () => {
    setShowHeaderPreview(false);
    setTempFilePath(pendingUploadData.temp_file_path);
    setDatasetName(pendingUploadData.dataset_name);
    setDescription(pendingUploadData.description || '');
    setOriginalFilename(pendingUploadData.original_filename);
    setFileSize(pendingUploadData.file_size_bytes);
    setCurrentStage(2);
  };
  ...
  const handleHeaderCancel = () => {
    setShowHeaderPreview(false);
    // Cleanup temp file
    if (pendingUploadData?.temp_file_path) {
      const formData = new FormData();
      formData.append('temp_file_path', pendingUploadData.temp_file_path);
  ...
  return (
    <div className="data-cleaning-page">
      ...
      <HeaderPreviewModal
        ...
        tempFilePath={pendingUploadData?.temp_file_path}
        hasHeader={pendingUploadData?.has_header ?? true}
        headersAutoGenerated={pendingUploadData?.headers_auto_generated ?? false}
        sessionToken={sessionToken}
      />
